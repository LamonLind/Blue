...
daily_usage=$(vnstat -d --oneline | awk -F\; '{print $8}' | sed 's/ //')
monthly_usage=$(vnstat -m --oneline | awk -F\; '{print $8}' | sed 's/ //')
...
echo -e "     ${BICyan}[${BIWhite}17${BICyan}] BANDWIDTH QUOTAS ${BICyan}${BIYellow}${BICyan}${NC}"
