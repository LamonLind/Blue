...
get_user_traffic() {
    ...
    local uplink_div3=$(echo $traffic_details | cut -d' ' -f1)
    local downlink_div3=$(echo $traffic_details | cut -d' ' -f2)
    
    # Return total: (uplink/3) + (downlink/3)
    echo $(( uplink_div3 + downlink_div3 ))
}
