...
get_user_traffic_detailed() {
    ...
    if [ -x "$_Xray" ]; then
        # Escape regex special characters in email (xray -pattern uses Go regexp,
        # so dots and plus signs must be escaped to avoid matching other users' stats)
        local escaped_email
        escaped_email=$(printf '%s' "$email" | sed 's/[.+*?^${}|]/\\&/g')
        
        # Query uplink (upload)
        uplink=$($_Xray api statsquery --server=127.0.0.1:10085 -pattern "user>>>$escaped_email>>>traffic>>>uplink" 2>/dev/null | sed -n 's/.*"value"[[:space:]]*:[[:space:]]*"\?\([0-9]\+\)"\?.*/\1/p' | head -1)
        [ -z "$uplink" ] && uplink=0
        
        # Query downlink (download) - handle both "value":"123" and "value": 123 formats
        downlink=$($_Xray api statsquery --server=127.0.0.1:10085 -pattern "user>>>$escaped_email>>>traffic>>>downlink" 2>/dev/null | sed -n 's/.*"value"[[:space:]]*:[[:space:]]*"\?\([0-9]\+\)"\?.*/\1/p' | head -1)
        [ -z "$downlink" ] && downlink=0
    fi
    
    # Return uplink and downlink as space-separated values
    echo "$uplink $downlink"
}

# Get user traffic from xray stats API
# Formula: uplink + downlink
get_user_traffic() {
    local email="$1"
    ...
    local downlink=${traffic_details##* }
