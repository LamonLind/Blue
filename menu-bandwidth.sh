...
# Escape regex special characters for xray stats pattern matching
escape_xray_pattern() {
    printf '%s' "$1" | sed 's/[][\\.^$|?*+(){}]/\\&/g'
}
...
get_user_traffic_detailed() {
    ...
    if [ -x "$_Xray" ]; then
        ...
        local escaped_email
        escaped_email=$(escape_xray_pattern "$email")
        
        # Query uplink (upload)
        uplink=$($_Xray api statsquery --server=127.0.0.1:10085 -pattern "^user>>>$escaped_email>>>traffic>>>uplink$" 2>/dev/null | sum_stats_values)
        [ -z "$uplink" ] && uplink=0
        
        # Query downlink (download) - handle both "value":"123" and "value": 123 formats
        downlink=$($_Xray api statsquery --server=127.0.0.1:10085 -pattern "^user>>>$escaped_email>>>traffic>>>downlink$" 2>/dev/null | sum_stats_values)
